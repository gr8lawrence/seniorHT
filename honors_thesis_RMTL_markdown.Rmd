...
pca.df <- list()
for (i in 1:num.studies) {
  pca.df[[i]] <- add_column(learning.df[[i]],
                            dataset = studies.names[i])
}
plot.df <- pca.df[[1]]
for (i in 2:num.studies) {
  plot.df <- rbind(plot.df, pca.df[[i]])
}

pca <- autoplot(prcomp(plot.df[,1:(dim(plot.df)[2]-2)]),
         data = plot.df,
         colour = "dataset")

ggsave(pca, filename =  "/Users/gr8lawrence/Desktop/Senior Honors Thesis/pca.jpeg")

pca2 <- autoplot(prcomp(plot.df[,1:(dim(plot.df)[2]-2)]),
         data = plot.df,
         colour = "dataset",
         shape = "class")

ggsave(pca2, filename =  "/Users/gr8lawrence/Desktop/Senior Honors Thesis/pca2.jpeg")
```
***
## Learning From Data
### Multi-task Learning
