...
### Figure 2
``` {r roc_curve_manual}
single_row_inds <- which(roc_df_tibble$strategy == "Single")
combined_row_inds <- which(roc_df_tibble$strategy == "Combined")
rf_tsp_roc_plot <- ggplot(roc_df_tibble[single_row_inds,], aes(x = false.positive, y = sensitivity)) +
                    geom_step(col = "#E69F00", show.legend = TRUE) + 
                    geom_step(data = roc_df_tibble[combined_row_inds,], aes(x = false.positive, y = sensitivity),
                              col = "#56B4E9",show.legend = TRUE) +
                    facet_grid(holdout.set~.) + theme_light() +
                    ggtitle("ROC Plor for Hold-out Analysis", subtitle = "Random Forest + TSPs")

ggsave(paste(plot_dir, "RF_TSPs_ROC_plot.jpg"))
rf_tsp_roc_plot
```
