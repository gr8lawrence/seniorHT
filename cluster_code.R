...
learning.results <- tibble(learning.set = rep('NA', 2*length(studies)^2), 
                           ...
                           gamma = rep(0, 2*length(studies)^2),
                           cost = rep(0, 2*length(studies)^2),
...
for (i in 1:len) {
  ...
  for (j in 1:length(studies.min.1)) {
    ...
    # First, we need to determine the tuning parameters so the SVM gains some sensitivity
    # Cross-validation generate 40 SVMs  
    tune <- tune.svm(class ~ ., data = learning.set, 
                     gamma = seq(1/dim(learning.set)[2], 3/dim(learning.set)[2], by = 0.5/dim(learning.set)[2]), 
                     cost = seq(0.5, 1, by = 0.1))
  ...
  # Support Vector Machine
  # We go through parameter tuning as well
  tune <- tune.svm(class ~ ., data = learning.set, 
                   gamma = seq(1/dim(learning.set)[2], 3/dim(learning.set)[2], by = 0.5/dim(learning.set)[2]), 
                   cost = seq(0.5, 1, by = 0.1))
  classical.basal.ratio = sum(learning.set$class == "classical")/sum(learning.set$class == "basal")
...
write.table(learning.results, file = "/nas/longleaf/home/tianyi96/learning_results.csv")

save(x = learning.results, file = "learning_result.Rdata")
save.image()
unlink("learning_result.Rdata")
