...
### MTL
Multitask (MTL) learning is a relatively new classification technique that weighs in the information from all training data for feature selections. In our study, we apply an implementation of the MTL as an analougue to the penalized logistic regression (with a specific L21 norm) with extra penalization for across-dataset feature selections to predict pancreatic cancer subtypes from genomic data. 
...
``` {r pred_accu_mtl}
ggplot(data = mtl_results_trimmed, aes(x = transformation, y = accuracy, fill = transformation)) +
  geom_boxplot(alpha = 0.8, width = 0.2) +
  ggtitle("Comparison of Prediction Accuracy",
