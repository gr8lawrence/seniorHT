...
for (i in 1:num.studies) {
  ...
  k <- 0 #index of the row needs to be recorded
  for (j in lam2s$lam2) {
    k <- k + 1
...
write.table(learning.results, file = "result_tables/MTL_TSPs_results.csv")
save(x = final_results, file = "models_and_predictions/MTL_TSPs_results.Rdata")
save(x = final_results_plr, file = "models_and_predictions/MTL_TSPs_no_gp_results.Rdata")
save(x = final_preds, file = "models_and_predictions/MTL_TSPs_predictions.Rdata")
save(x = final_preds_plr, file = "models_and_predictions/MTL_TSPs_predictions_no_gp.Rdata")
save.image()
